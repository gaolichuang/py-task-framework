...
class Resource(wsgi.Application):
    ...
    def get_body(self, request):
        try:
            LOG.debug(_("Get Request Info : %s"%request))
...
#            return None, ''
    ...
    def __call__(self, request):
        ...
        LOG.debug('Request evn is %s '% request.environ)
        action = action_args.pop('action', None)
        content_type, body = self.get_body(request)
        accept = request.best_match_content_type()
        LOG.debug(_('Resource __call__ action_args: %(action_args)s;'
                              'action:%(action)s; accept:%(accept)s;'
                              'content_type:%(content_type)s;, body:%(body)s;'),
                              {'action_args': action_args, 'action': action,
                               'content_type':content_type,'body':body,'accept':accept})
